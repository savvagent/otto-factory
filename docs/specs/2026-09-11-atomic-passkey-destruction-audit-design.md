...
A third, related gap lives in `of_web::routes::orgs::reset_member_passkeys`: after its own
already-atomic, org-scoped `tx.audit(MEMBER_PASSKEYS_RESET)` write commits, it writes a *second*,
best-effort, `org_id = NULL` `auth.passkey.cleared` row on `db.pool()`, attributed to the admin.
That write is loseable today, and (per §3 below) it is structurally incapable of being folded into
the pinned transaction that precedes it while staying `org_id = NULL` — RLS's
`audit_events_append` policy (`WITH CHECK (current_org() IS NULL OR org_id = current_org())`,
`0008_audit.sql`) rejects a `NULL`-org insert on a connection pinned to a non-null org whenever RLS
applies. The only way to make this write atomic with the destructive change it records is to give
it the real `org_id` and write it through `Tx::audit` inside the existing transaction — which is
exactly what the issue proposes, and which has the added benefit of finally putting this event in
front of the org admins who need to review it (today it is invisible to `GET /api/orgs/{org}/audit`,
since that endpoint filters on `org_id = $1` and this row's `org_id` is `NULL`).

- `passkeys::remove` opens `db.begin_unpinned()`, runs the last-passkey check, the `DELETE`, and the
...
- `of_web::routes::orgs::reset_member_passkeys` moves its post-commit, best-effort, `org_id = NULL`
  `auth.passkey.cleared` write into its existing pinned transaction as a second `tx.audit(...)` call
  (real `org_id`, same as the `MEMBER_PASSKEYS_RESET` row already written there), before
  `tx.commit()`. The post-commit best-effort block is deleted.
- A failure on any of these audit writes now aborts the whole operation and returns `Err`, instead of
  being swallowed into `tracing::error!` while the destructive change still commits. This is a
  deliberate, `#131`-precedented behavior change — see "Public-interface changes" below.
- `cargo test --workspace`, `cargo clippy --all-targets -- -D warnings`, and
  ...
  one needs a small, deliberate assertion change (§4) reflecting that the `auth.passkey.cleared` row
  it writes is now org-scoped rather than global, which is the point of the fix.
- Three new forced-failure tests (mirroring `#131`'s `a_forced_audit_failure_rolls_back_the_credential`)
...
- `of_web::routes::orgs::reset_member_passkeys` (`crates/of-web/src/routes/orgs.rs`): move its
  post-commit `auth.passkey.cleared` write into the existing pinned transaction, org-scoped via
  `Tx::audit`, replacing the post-commit `db.audit_global(...)` call.
- New forced-failure tests for all three call sites, per `#131`'s established technique.
- The one existing test (`an_admin_can_reset_a_members_authenticator_but_gains_nothing_by_it`) whose
  assertion is a direct consequence of §3's org-scoping change.

**Out:**

- **No change to `passkeys::remove`'s or `passkeys::clear`'s public signatures.** Same parameters,
  same `Result<()>` / `Result<u64>` return types.
- **No fix for the last-passkey count check's TOCTOU race** (`count(db, user).await?` then a
  separate `DELETE`, both currently unserialized against a concurrent `remove` on the same account).
  This predates this change, is not what `#134` reports, and moving both statements onto the same
  transaction's connection narrows the window (same session, `READ COMMITTED`) without closing it
  entirely (a genuine fix needs `SELECT ... FOR UPDATE` or a check constraint). Filed as a
  follow-up rather than folded in here, per this repo's own precedent for out-of-scope findings
  (`#132`/`#133`/`#134` themselves, filed out of `#131`'s scope) and per Stop & Escalate condition 11
  in `otto-factory-development` ("the same bug pattern is discovered elsewhere — file a follow-up
  issue; do NOT silently widen scope").
...
- **No change to `MEMBER_PASSKEYS_RESET`'s write** — it was already atomic and org-scoped before
  ...
  touches the *second*, best-effort `auth.passkey.cleared` write in the same function.
...
|---|---|---|
| `of_auth::passkeys::remove` (crate-public fn, workspace-internal caller only: `of_web::routes::auth::remove_passkey`) | Same signature. On an audit-write failure, now returns `Err` (rolling back the `DELETE`) where it previously returned `Ok(())` with a logged, swallowed audit-write failure and a *committed* delete | Behavioral, not a signature change. The one caller already propagates `remove`'s `Result` via `?`, so no caller code changes; only what triggers the existing error path changes, and the *outward* effect (a passkey either both gets removed and gets audited, or neither) is now consistent with what an admin reviewing the trail would expect |
...
| `of_web::routes::orgs::reset_member_passkeys` (HTTP handler behind `POST /api/orgs/{org}/members/{user}/reset-passkeys`) | Same request/response shape (`201 { code, link }` on success). The `auth.passkey.cleared` audit row it writes now carries the org's real `org_id` instead of `NULL`, and is written atomically inside the same transaction as the `DELETE`/session-revoke/claim-insert, instead of best-effort afterward. A transient failure on this write now fails the whole request (existing 500-class mapping) instead of silently succeeding with a lost audit row | Behavioral: (1) the response shape and the already-atomic `MEMBER_PASSKEYS_RESET` row are unchanged; (2) the `auth.passkey.cleared` row's `org_id` column changes from always-`NULL` to the acting org's id for this one write path — additive from the reader's point of view (a row that was invisible to `GET /api/orgs/{org}/audit` now appears there, filtered on `action=auth.passkey.cleared`), and no existing consumer is documented to rely on this specific row's `org_id` being `NULL` (see Risks & Open Questions) |
| `of_core::audit::Db::audit_global_on` | No change — reused as-is (already takes `&mut Unpinned`, per `#165`) | No |
| `of_core::audit::Tx::audit` | No change — reused as-is for the additional call in §3 | No |
| MCP | none | — |
| Console REST | `GET /api/orgs/{org}/audit?actionPrefix=auth.passkey.cleared` can now return rows for admin-assisted resets against that org, where before it returned none for that action (they were written with `org_id = NULL` and therefore invisible to any org-scoped read). This is the fix's own point (see Goal) and is additive, not a shape change | No |
...
New shape, mirroring `finish_registration`'s `_tx` split so a future caller that needs to fold
another change into the same commit (there is none today — see Assumptions) has the same shape
available:
...
/// `finish_registration_tx` does for `claim_finish` — but split out in the same
...
untouched — `remove_tx` runs its own `SELECT count(*)` on the transaction's connection instead of
calling it, since `count` takes `&Db` (pool-bound) and cannot run on a caller-held transaction.
Duplicating one `SELECT count(*)` string is preferable to changing `count`'s signature for one
internal caller.
...
## §3 `reset_member_passkeys`'s `auth.passkey.cleared` write becomes org-scoped and atomic
...
The fix the issue proposes resolves this directly: write it with the real `org_id`, via `Tx::audit`,
inside the same transaction as the other three statements:
...
// action string sees an admin-assisted clear too — but scoped to the real
// org rather than NULL, because this is a pinned transaction (see the long
// comment this replaces) and because a NULL-org row here would be invisible
// to the very org admins reviewing this event, which defeats the point of
// writing it at all. See savvagent/otto-factory#134.
tx.audit(
    Entry::new(action::PASSKEY_CLEARED)
        .actor(ctx.user.id)
        .target("user", target.to_string())
...
The post-commit `if let Err(e) = state.db.audit_global(...) { tracing::error!(...) }` block is
deleted entirely — both audit rows now commit or roll back with the destructive change, in the same
transaction.

This produces two rows per reset under two different action names
(`org.member.passkeys_reset` and `auth.passkey.cleared`), both scoped to the same org, both
attributed to the acting admin. That redundancy is deliberate, not accidental: `MEMBER_PASSKEYS_RESET`
is the org-administration event (visible under `actionPrefix=org.member`), and `PASSKEY_CLEARED` is
the same wire action a self-service clear would write (`of_auth::passkeys::clear`, §2), so a
customer filtering their SIEM export on `auth.passkey.cleared` sees every account-clearing event —
self-service and admin-assisted alike — under one action name, which was already the reason this
second write existed before this change (see the deleted comment's own text: "attributed to the
admin who did this, not the member it happened to"). This section only changes *how* it is made
durable, not *whether* it is written or what it says.

## §4 Test updates

**Updated:** `an_admin_can_reset_a_members_authenticator_but_gains_nothing_by_it`
(`crates/of-web/tests/console.rs:1710`). Its existing assertion:
...
becomes (querying by the acme org's own id instead of `org_id IS NULL`, since that is exactly what
§3 changes):

```rust
let acme = h.db.get_org_by_slug("acme").await.unwrap().unwrap();
let cleared: (Option<String>, Option<String>) = sqlx::query_as(
    "SELECT actor_user_id::text, target_id FROM audit_events \
...
The row's content (actor = Rob, target = Bob) and the surrounding comment explaining the
attribution are unchanged; only the scoping predicate and the comment noting *why* it is now
org-scoped change.

**New:** three forced-failure tests, each installing a `BEFORE INSERT ... WHEN (NEW.action = ...)`
trigger on `audit_events` that raises an exception, mirroring
...
- `a_forced_audit_failure_rolls_back_an_admin_assisted_reset` (`crates/of-web/tests/console.rs`):
  trigger on `action = 'auth.passkey.cleared'`; `POST
  /api/orgs/{org}/members/{user}/reset-passkeys`; assert the request fails (existing 500-class
  mapping); assert the target's passkey, session, and the absence of any claim row all reflect that
  nothing committed — i.e. `MEMBER_PASSKEYS_RESET` also did not commit, proving §3's whole
  transaction (delete + session revoke + claim insert + both audit rows) is one atomic unit, not
  three independent ones that happen to run in sequence.

## Testing

- `cargo test -p of-auth --test passkeys` — existing suite plus the two new forced-failure tests.
...
- **The `PASSKEY_CLEARED` row `reset_member_passkeys` writes is kept, not dropped, despite being
  redundant with `MEMBER_PASSKEYS_RESET`.** The issue's fix sketch asks to re-scope this write, not
  to remove it, and the pre-existing test's comment ("The global auth.passkey.cleared row is
  attributed to Rob...") indicates it was a deliberate cross-action-name mirror for SIEM consumers
  keyed on `auth.passkey.cleared` rather than an oversight. Removing it would be a larger,
  un-asked-for behavior change (an existing action string a customer may already alert on would stop
  appearing for admin-assisted resets). If this assumption is wrong, dropping the redundant write
  entirely (keeping only `MEMBER_PASSKEYS_RESET`) is a strictly smaller diff than this spec proposes.
...
- **The last-passkey count-then-delete race (`remove`) is not fixed here** — see Scope/Out. Worth a
  follow-up if it is ever observed in practice; not a forensic-audit gap, so out of `#134`'s own
  scope.
- **Two audit rows per admin-assisted reset (`MEMBER_PASSKEYS_RESET` and `PASSKEY_CLEARED`) is
  acceptable but should be confirmed with the architect reviewer** — see Assumptions above. Flagged
