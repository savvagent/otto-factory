...
- [ ] Create the directory `.claude/` if it doesn't already exist as a tracked path.
- [ ] Create `.claude/skills` as a symlink to `../.github/skills` (relative, so it resolves the
      same way regardless of where the repo is cloned): `ln -s ../.github/skills .claude/skills`.
- [ ] Verify it resolves: `ls .claude/skills/otto-factory-development/SKILL.md` must show the real
      file, and `readlink .claude/skills` must print `../.github/skills`.
- [ ] Verify git tracks it as a symlink, not a copy: `git add .claude/skills && git status` should
...
- [ ] Confirm no other repo tooling treats `.claude/` specially in a way this would break: `grep -rn
      "\.claude/" --include="*.yml" --include="*.yaml" .github/workflows/` should show nothing (CI
      does not reference `.claude/`).
- [ ] Format and commit: `git add .claude/skills && git commit -m "docs: symlink .claude/skills to .github/skills for Claude Code discovery"`.

## Final gate

- [ ] `cargo test --workspace` — vacuously satisfied, no Rust source changed; state this explicitly
