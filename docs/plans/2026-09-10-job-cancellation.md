...
Done. All four tasks shipped in savvagent/otto-factory#93, including two follow-up
review-response rounds: the first added a `JobTransition::Cancelled` variant
...
push — CI run 34435078581's `deploy` job succeeded, and `of-server` migrates at boot, so
`0023`/`0024` are confirmed applied against the real production database (also verified
directly post-deploy: a live `add_job` call returned the new `cancelRequestedAt`/
`cancelRequestedBy`/`cancelReason` fields).
...
| **Modify.** `crates/of-mcp/tests/tools.rs` | End-to-end tool tests, tool-list/billing assertions, audit-trail assertions (post-review). |
...
| **Modify.** `web/src/lib/types.ts` | `JobStatus`/`QueueStats` gain `cancelled`; `Job` gains `cancelRequestedAt`/`cancelRequestedBy`/`cancelReason` (post-review). |
...
| **Modify (post-review).** `crates/of-trackers/src/sync.rs` | Adds `JobTransition::Cancelled` (no `github_close`, no `jira_transition`) — not in the original plan; see Task 2's correction above. |
| **Modify (post-review).** `crates/of-trackers/tests/sync.rs` | Test fixture update, plus `outbound_decision` coverage for `JobTransition::Cancelled`. |
| **Modify (post-review).** `web/src/routes/o/[org]/queue/[job]/+page.svelte` | Surfaces `cancelReason`/`cancelRequestedAt` on the job detail page — not in the original Task 4 file list; added in response to review finding the console never actually rendered the fields Task 3 exposed. |
...
- [x] Write `crates/of-core/migrations/0023_job_cancelled_status.sql` exactly as spec §2
...
- [x] Write `crates/of-core/migrations/0024_job_cancellation_columns.sql` exactly as spec
      §2 (three nullable columns, `cancel_requested_by` a `REFERENCES users (id) ON DELETE
      SET NULL`).
- [x] Run `cargo test -p of-core` once with no other code changes yet, purely to prove a
...
- [x] Run `cargo test -p of-core --test queue` — expect compile failure (the new `Status`
      variant, fields, and `Tx` methods do not exist yet).
- [x] Implement `crates/of-core/src/jobs.rs` per spec §3: `Status::Cancelled` variant;
...
- [x] In `crates/of-core/src/audit.rs`, add the `// Jobs.` section to `pub mod action`
...
- [x] In `crates/of-core/tests/isolation.rs`, extend `cross_org_mutation_is_refused`
...
- [x] Run `cargo test -p of-core --test isolation` — expect all pass, including the two
      new assertions.
- [x] `cargo clippy --all-targets -- -D warnings` clean. `cargo fmt --all`.
- [x] Commit: `git commit -m "of-core: add job cancellation (request_cancel, cancel_job, Cancelled status)"`.
...
- [x] Run `cargo test -p of-mcp --test tools` — expect compile failure (tools do not exist
      yet) or, if it compiles against stubs, straightforward assertion failures.
- [x] Implement `RequestCancelArgs`/`CancelJobArgs` and the `request_cancel`/`cancel_job`
      ...
      - The post-commit `sync_jobs_after_transition(..., JobTransition::Failed,
        ...
        job). **(Post-review, superseded — do not implement this as written.)** PR review
        found `JobTransition::Failed`'s existing `(Failed, Jira)` outbound mapping
        transitions a linked JIRA ticket to the "new" status category, which is backwards
        for a cancellation ("still needs doing" about work someone just asked to stop).
        The shipped code instead adds a dedicated `JobTransition::Cancelled` variant to
        `crates/of-trackers/src/sync.rs` (no `github_close`, no `jira_transition`, same
        comment-fallback behavior) and passes that variant at both call sites and in
        `sync_ticket`'s arm below. See the corrected spec §4/Scope-Out and commit
        `3f054be`.
      - `sync_ticket`'s match over `job.status` gains a `Status::Cancelled` arm — per the
        correction above, this passes `JobTransition::Cancelled`, not `JobTransition::Failed`.
      - `require_scope(scope::JOBS_WRITE)` on both new tools, matching every other
        job-mutating tool.
- [x] In `crates/of-billing/src/classify.rs`, add `"request_cancel"` and `"cancel_job"` to
...
- [x] Update tool descriptions per spec §4: `repend_job`'s description text ("Return a
...
- [x] `cargo clippy --all-targets -- -D warnings` clean. `cargo fmt --all`.
- [x] Commit: `git commit -m "of-mcp: add request_cancel and cancel_job tools"`.
...
- [x] In `crates/of-web/src/openapi.rs`'s test module (wherever the existing `"active"`
...
- [x] Run the relevant `cargo test -p of-web` test — expect failure (schema literal not
      yet updated).
- [x] Update `queue_schemas()` in `openapi.rs` exactly as spec §7: the three new `Job`
      properties, the status enum's `"cancelled"` entry, `QueueStats`'s `"cancelled"`
      property and `required` entry.
- [x] Update `routes/jobs.rs`'s `ListJobsQuery.status` doc comment to list `cancelled`.
- [x] Run the `cargo test -p of-web` schema tests — expect pass. Then run
...
- [x] `cargo clippy --all-targets -- -D warnings` clean. `cargo fmt --all`.
- [x] Commit: `git commit -m "of-web: surface job cancellation fields in the console schema"`.
...
- [x] In `web/messages/en.json`, add `"status_cancelled": "Cancelled"` immediately after
...
- [x] Run `cd web && npm run check` — expect it to still pass at this point (an unused
      catalog key is not an error).
- [x] In `web/src/lib/types.ts`: `JobStatus` gains `'cancelled'`; `QueueStats` gains
      `cancelled: number`.
- [x] Run `npm run check` — expect failures at every place TypeScript now requires a
...
- [x] In `web/src/routes/o/[org]/queue/+page.svelte`, add `'cancelled'` to the end of the
      `STATUSES` array.
- [x] Run `npm run check && npm run lint && npm test` — expect all pass. `npm test`
...
- [x] Commit: `git commit -m "web: recognize the cancelled job status"`.
