...
✅ Shipped in `savvagent/otto-factory#74`, merged as `14c9fc9`, deployed by the `deploy` job on that
push and verified live. Tasks 1–4 and 6–8 are done; **Task 5b was implemented and then reverted**
(see its section). Closes `savvagent/otto-factory#59` and `#60`.

Review found more than the tasks did, and the pattern is the durable part of this record. The
independent security review — given only the diff, without this plan, the spec, the issues or the PR
body — is what showed Task 5b's throttle was a shared-IP denial of service on the only sign-in path,
after two earlier reviewers had accepted it. Five stale comments and route descriptions were found
by reviewers rather than by the tasks that invalidated them, and two of those were written *during*
this change while fixing something else. A revert staged with `git add -A` swept up a concurrent
reviewer's mutation experiment and deleted three feature-detection guards; three reviewers caught it
independently.

Deferred, filed rather than folded in: `#64` (`TeamMember.email` non-null over a nullable column),
`#72` (OpenAPI drift, and the missing gate that let it persist), `#73` (a render test that fails
about half of runs), `#75` (throttling `login/finish` safely, plus `ratelimit`'s non-atomic
check/record), `#76` (passkey audit events filed under TOTP action names, with discarded writes).
...
- [x] Write the failing tests first, as bare `#[sqlx::test]` cases in a new
...
- [x] Run `cargo test -p of-core --test labels` — expect compile failure (no `labels` module, no
      `label` field).
- [x] Create `crates/of-core/migrations/0022_user_label.sql`:
...
- [x] Create `crates/of-core/src/labels.rs` with `pub fn generate() -> String`: 80 adjectives × 80
...
- [x] In `crates/of-core/src/orgs.rs`: add `pub label: String` to `User` with a doc comment saying
...
- [x] Run `cargo test -p of-core --test labels` — expect green.
- [x] Run `cargo test --workspace` — expect green (other crates may need `label` in `User`
      constructions; fix any that do not compile).
- [x] Verify a fresh cluster applies cleanly: `podman compose down -v && podman compose up -d` then
      `cargo test -p of-core`.
- [x] `cargo fmt --all`, `cargo clippy --all-targets -- -D warnings`, commit as
...
- [x] Add failing tests to `crates/of-auth/tests/passkeys.rs`:
...
- [x] Run `cargo test -p of-auth --test passkeys` — expect failure.
- [x] In `crates/of-auth/src/passkeys.rs`: add `const RP_NAME: &str = "otto-factory";` and use it in
      ...
      as they are.
- [x] Run `cargo test -p of-auth --test passkeys` — expect green.
- [x] `cargo fmt --all`, `cargo clippy --all-targets -- -D warnings`, `cargo test --workspace`,
...
- [x] Add failing tests to `crates/of-web/tests/console.rs`:
...
- [x] Run `cargo test -p of-web --test console` — expect failure.
- [x] Add `auth::webauthn_config`: `State(state)` only, no extractor, returning
...
- [x] Mount it in `catalog.rs` as `Endpoint::get("/api/auth/webauthn", auth::webauthn_config)
...
- [x] Add `credential_name` and `credential_display_name` to the `Me` struct in
...
- [x] `openapi.rs`: add the new `WebauthnConfig` component, and add `label` to `User`, `OrgMember`
...
- [x] **While in `response_schemas()`, correct the two stale schemas it holds.** `Me` and
...
- [x] Run `cargo test -p of-web --test console` — expect green, including
...
- [x] Add a failing test to `crates/of-web/tests/console.rs`: after an account registers a passkey,
...
- [x] Run `cargo test -p of-web --test console` — expect failure.
- [x] Add `pub credential_id: String` to `RegisteredKey` (serialized `credentialId`), select
...
- [x] Add `credentialId` to the `Passkey` schema in `openapi.rs` — `properties` **and** `required`.
      (`OrgMember` and `TeamMember` gained `label` in Task 3's schema step, alongside `User`.)
- [x] Run `cargo test -p of-web --test console` and `cargo test -p of-auth` — expect green.
- [x] `cargo fmt --all`, `cargo clippy --all-targets -- -D warnings`, commit as
...
- [x] Run `cargo test -p of-auth --test passkeys` — expect failure.
- [x] In `finish_authentication`, return `AuthError::UnknownCredential` on the `owner == None`
...
- [x] **Correct the two comments that now contradict the code.** Neither is load-bearing on a test
...
- [x] Confirm no other of-web change is needed: `AuthError::UnknownCredential` already maps to
...
- [x] Run `cargo test -p of-auth --test passkeys` and `cargo test --workspace` — expect green.
- [x] `cargo fmt --all`, `cargo clippy --all-targets -- -D warnings`, commit as
      `of-auth: say when a presented credential is one this server never stored`.

## Task 5b — Throttle `login/finish` — **implemented, then reverted** ✅ (nothing shipped)
...
- [x] `cd web && npm install` if `node_modules` is absent.
- [x] Write `web/src/lib/webauthn.test.ts` first (plain vitest, no jsdom needed — stub
      ...
      - A helper does nothing when the rp_id could not be fetched, rather than falling back to
        `location.hostname`.
- [x] Run `npx vitest run src/lib/webauthn.test.ts` — expect failure.
- [x] Add `WebauthnConfig` to `types.ts` and `webauthnConfig: () => get<WebauthnConfig>('/api/auth/webauthn')`
...
- [x] Implement in `web/src/lib/webauthn.ts`: `userHandle(uuid)`; a module-level cached
...
- [x] Run `npx vitest run src/lib/webauthn.test.ts` — expect green.
- [x] `npm run check`, `npm run lint`, `npm test` — expect green. Commit as
...
- [x] Signal call sites, each **after** the existing `session.refresh()`, each `await`ed but unable
...
- [x] Render the label where the console names an account with no address:
...
- [x] Delete from all six `web/messages/*.json` the keys this leaves unreferenced — expected to be
...
- [x] Update any fixture or render test that constructs a `User`, `OrgMember`, or `Passkey`
...
- [x] `npm run check` (which runs `scripts/check-messages.mjs` first), `npm run lint`, `npm test`,
      `npm run build` — all four green.
- [x] Commit as `web: signal credential changes and show the account label`.
...
- [x] Open the PR with `Closes #59` and `Closes #60`, and name in the body:
      ...
      3. **`savvagent/otto-factory#64` deferred, deliberately** (Task 1) — `TeamMember.email` is
         ...
         change.
- [x] After merge, flip the spec's `> **Status:**` to `IMPLEMENTED`, mark every task above ✅, and
