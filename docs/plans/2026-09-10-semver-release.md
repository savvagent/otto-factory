...
- [x] Create `release-please-config.json` at the repo root with exactly the content from spec §1:
...
- [x] Create `.release-please-manifest.json` at the repo root: `{".": "0.1.0"}` — matches the
...
- [x] Create `CHANGELOG.md` at the repo root, content exactly `# Changelog\n`.
- [x] Validate all three parse: `python3 -c "import json; json.load(open('release-please-config.json')); json.load(open('.release-please-manifest.json'))"`.
- [x] Note in the PR body: `web/package-lock.json`'s own root `"version"` field is not covered by
...
- [x] Format and commit: `git add release-please-config.json .release-please-manifest.json CHANGELOG.md && git commit -m "ci: add release-please config"`.
...
- [x] Append the `release-please` job from spec §1 after the existing `docker-build` job:
...
- [x] Append the `pr-title` job from spec §2: `if: github.event_name == 'pull_request'`,
      ...
      `timeout-minutes: 5`.
- [x] Change `deploy`'s `needs:` from `[rust, web, docker-build]` to `[release-please]` and its
...
- [x] Validate: `python3 -c "import yaml; yaml.safe_load(open('.github/workflows/ci.yml'))"`.
- [x] Confirm by inspection (no local GitHub Actions runner available) that `pr-title`'s config
      ...
      command.
- [x] Format and commit: `git add .github/workflows/ci.yml && git commit -m "ci: gate deploy on a release-please release, add PR title lint"`.
...
- [x] Write the failing test first in `crates/of-mcp/tests/tools.rs`, alongside the existing
...
- [x] Implement: in `crates/of-mcp/src/server.rs`'s `get_info()`, add
...
- [x] Run the test again and confirm it passes: `cargo test -p of-mcp --test tools`.
- [x] Run the full crate suite to catch any other assertion on `get_info()`'s output:
      `cargo test -p of-mcp`.
- [x] Format and commit: `cargo fmt --all && git add crates/of-mcp/src/server.rs crates/of-mcp/tests/tools.rs && git commit -m "of-mcp: report otto-factory's own name and version at initialize, not rmcp's"`.
...
- [x] Add a test asserting `openapi::document()`'s `info.version` equals
...
- [x] Run the full crate suite: `cargo test -p of-web`.
- [x] Format and commit: `cargo fmt --all && git add crates/of-web/tests/console.rs && git commit -m "of-web: pin the OpenAPI document's version to the workspace version"`.
...
- [x] Add the "Releases & versioning" section to `CLAUDE.md`, placed after `## Style` (its current
      last section), with exactly the content from spec §5.
- [x] Update `docs/deploy/fly.md`: add a paragraph stating deploys now happen when the
...
- [x] Add the one-line "Superseded in part" pointer from spec §4 to
...
- [x] Add a one-line note to `docs/clients/matrix.md` recording the `server_info` fix, phrased as a
      ...
      structure.
- [x] Format and commit: `git add CLAUDE.md docs/deploy/fly.md docs/specs/2026-09-08-master-autodeploy-design.md docs/clients/matrix.md && git commit -m "docs: record semver release automation"`.
...
- [x] `python3 -c "import json; json.load(open('release-please-config.json')); json.load(open('.release-please-manifest.json'))"`
- [x] No `web/` source changed by this plan, so `npm run check`/`lint`/`test` are vacuously
