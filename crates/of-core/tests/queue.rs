...
async fn org_wide_stats_terminal_counters_track_every_transition(pool: PgPool) {
    ...
    .await
    .unwrap();
    tx.complete_job(&completed.id, None).await.unwrap();
    tx.fail_job(&failed.id, Some("boom")).await.unwrap();
    tx.complete_job(&repended.id, None).await.unwrap();
    tx.complete_job(&deleted.id, None).await.unwrap();
    ...
    let never_finished = tx
        ...
        .await
        .unwrap();
    tx.claim_jobs(std::slice::from_ref(&never_finished.id), t.user, None)
        .await
        .unwrap();
...
async fn repo_scoped_stats_match_the_org_wide_counters(pool: PgPool) {
    ...
    let mut tx = db.begin(t.org).await.unwrap();
    let a = tx.add_job(job(&t, "a")).await.unwrap();
    tx.claim_jobs(std::slice::from_ref(&a.id), t.user, None)
        .await
        .unwrap();
    tx.complete_job(&a.id, None).await.unwrap();
